...
- Prefer these scripts over manually transcribing a deck. If a deck comes from elsewhere (e.g. TappedOut), add a new `scripts/<site>_extract.py` following the same normalize-then-handoff pattern rather than transcribing by hand.
...
- **Scryfall** — direct fetches to `api.scryfall.com`/`scryfall.com` return 403. Instead `WebFetch` `https://r.jina.ai/https://api.scryfall.com/cards/named?fuzzy=<card+name>` — returns exact oracle text (both faces for MDFCs), mana cost, and `prices.usd` cleanly. This is now the preferred method over `WebSearch`.
...
- When adding a new deck to the archive from Archidekt or Moxfield, always use the matching `scripts/*_extract.py` (see "Decklist extraction" above) instead of transcribing it by hand.
