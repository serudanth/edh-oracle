...
- `tools/moxfield_extract.py <url-or-deck-id>` / `--user <username>` — Moxfield. Moxfield has **no official public API**; this uses the same unauthenticated `api2.moxfield.com` endpoints moxfield.com's own frontend calls (`/v3/decks/all/{id}` for a deck, `/v2/decks/search-sfw?authorUserNames=` for a user's public decks), reverse-engineered via the [Aleqsd/moxfield-api](https://github.com/Aleqsd/moxfield-api) wrapper's source rather than guessed — if these ever break, re-check that project (or another Moxfield API wrapper) for the current paths before re-guessing blind.
  - `analysis/<deck-name-slug>.json` — Automated analysis output generated by `tools/deck_analyzer_engine.py` containing deterministic metrics, power scores, brackets, and combo lines.
- `tools/external_analysis_extract.py` — External deck-analysis extraction & normalization sub-tool. Normalizes external analysis scores and text (from ScryCheck, EDHCheck, RateMyDecks, CommanderPowerMeter, ArcMind, PowerDeckAI) into a standardized JSON output schema. Prompt specification lives in `tools/prompts/external_analysis_agent.md`.
- `tools/deck_analyzer_engine.py` — Local EDH deck analyzer & metric calculation engine. Performs piloting pattern classification, dynamic archetype weighting, sub-pillar scoring, and combo resolution via Scryfall cache + Commander Spellbook API. Output is written to `knowledgebase/podlist/<owner>/analysis/<deck-slug>.json`.
